...
echo "[2/4] copying the patched pipeline + STT"
scp -o IdentitiesOnly=yes -o StrictHostKeyChecking=no -i /home/owner/.ssh/id_dgx_spark "$SRC" "$DGX":cora/phase1_push_to_talk.py
scp -o IdentitiesOnly=yes -o StrictHostKeyChecking=no -i /home/owner/.ssh/id_dgx_spark "$(dirname "$SRC")/stt_deepgram.py" "$DGX":cora/stt_deepgram.py

echo "[3/4] compile check on the DGX venv"
"${SSH[@]}" "$DGX" '~/cora/venv/bin/python -m py_compile ~/cora/phase1_push_to_talk.py ~/cora/stt_deepgram.py && echo compile-ok'
